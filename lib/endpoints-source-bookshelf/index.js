...
Source.prototype.allowedFor = function (requestor, cb) {
  if (!this.model.allowedFor) {
    process.nextTick(function () {
      cb();
    });
  } else {
    this.model.allowedFor(requestor).exec(cb);
  }
};
...
Source.prototype.read = function (filters, relations, opts, cb) {
  ...
  // map include items in the url to the actual relation names
